...
// sends changed_by — the parent no longer needs to pass an email.
...
export function ManageMilestonesDialog({ brands, initialBrandId }: ManageMilestonesDialogProps) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      // The duplicate-detection lookup stays client-side: SELECT is RLS-
      // permitted for any authenticated user, and the restore-vs-create
      // decision needs a confirm() prompt before the mutation fires.
      ...
      const result: { ok?: boolean; restored?: boolean; error?: string; auditError?: string } =
        await res.json().catch(() => ({}));

      if (!res.ok || !result.ok) {
        if (res.status === 409) {
          toast(`❌ ${result.error ?? `A milestone already exists for ${ticket}.`}`);
        } else {
          console.error('[milestones] save failed', result.error);
          toast(`❌ ${result.error ?? 'Failed to save milestone'}`);
        }
        return;
      }
      ...
      toast(result.restored ? '♻️ Milestone restored' : '✅ Milestone added');
      setAddTicket('');
      setAddNotes('');
  ...
  async function saveEdit(m: Milestone) {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/milestones/${m.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reached_at: reachedAtIso, notes: nextNotes }),
      });
      const result: { ok?: boolean; changed?: number; error?: string; auditError?: string } =
        await res.json().catch(() => ({}));
      if (!res.ok || !result.ok) {
        console.error('[milestones] update failed', result.error);
        toast(`❌ ${result.error ?? 'Failed to save changes'}`);
        return;
      }
      if (result.auditError && process.env.NODE_ENV !== 'production') {
        toast(`⚠️ Audit write failed: ${result.auditError}`);
  ...
  async function confirmDelete() {
    ...
    const res = await fetch(`/api/admin/milestones/${deletingId}`, {
      method: 'DELETE',
    });
