...
# ▸ APPEND BLOCK — second extraction pass, 2026-08-23

> **⚠ THIS FILE IS APPEND-ONLY HISTORY. IT IS NEVER AUTHORITY FOR CURRENT
> STATE (§13 r40).** Every state read — what is deployed, what is pending,
> what a rule requires — resolves against `CLAUDE.md`, and against
> `docs/schema.md` for schema. Everything below was true as a record on its
> date and is stale as a claim. **Do not cite it as evidence for a
> present-tense claim, and never edit an entry here to make it current.**

**Relocated from CLAUDE.md §15 / §0 / §0.1 on 2026-08-23** by the second
extraction pass (board batch #1, rev 8.1) under §13 r42 clause 3: §15 keeps the
action plus a one-line citation, post-mortem narrative moves here. **Nothing
was deleted** — every block below is byte-identical to what left CLAUDE.md,
sliced programmatically at `ab70878`. **Live scope did NOT come here** — it
went to `docs/specs/`, which r40 does not cover.


---

## Old §0 `Prod right now` stanza — superseded 2026-08-23; prod had moved to `ab70878` two days earlier
*(CLAUDE.md lines 14–24 at `ab70878`)*
...
## Old §0.1 need-ordered priority list and its 2026-07-15 rationale — superseded by board rev 8.1 dependency order
*(CLAUDE.md lines 55–85 at `ab70878`)*
...
## Batch 012 — Convert reconciliation run-procedure repoint history (three repoints, two wrong)
*(CLAUDE.md lines 1768–1793 at `ab70878`)*
...
## Batch 012 — the "restyle batch 4" block and the VINTAGE counts narrative (batch DISSOLVED in board rev 8)
*(CLAUDE.md lines 1794–1814 at `ab70878`)*
...
## Batch 012 — Convert reconciliation resolver bug, full case detail (MDG/MOJ collisions, corrected-output accounting)
*(CLAUDE.md lines 1831–1879 at `ab70878`)*
...
## Batch 012 — stale §15.5 zero-cell note (closed by relocation: the entry it referenced left §15.5 for §16 under r34)
*(CLAUDE.md lines 1888–1892 at `ab70878`)*
...
## Batch 012 — PROCESS: spec-is-commit-1, original wording
*(CLAUDE.md lines 1909–1913 at `ab70878`)*
...
## Batch 012 — Tailwind-v4 token collision blast radius and the "Largest gap" KPI rationale
*(CLAUDE.md lines 1928–1940 at `ab70878`)*
...
## Batch 012 — matrix-controls archive-UI signal obligation (Karen LOW-8), CLOSED by 012 directive CRUD
*(CLAUDE.md lines 1942–1961 at `ab70878`)*
...
## Batch 012 — V2.1 trigger backport: why the loader was abandoned, and the EXPECTED-totals lesson now generalised to §13 r38
*(CLAUDE.md lines 1963–2002 at `ab70878`)*
...
## Batch 012 — prod directive-count drift narrative (69 → 75, 2026-07-29)
*(CLAUDE.md lines 2035–2043 at `ab70878`)*
