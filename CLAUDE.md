...
[input has '@' → email, else case-insensitive username→email via
user_profiles, legacy @cqip.local synthesis kept as TODO fallback];
assertTargetIsReadOnlyOrSelf-guarded set_email PATCH [ordered two-write,
retry-once, loud-fail, no rollback; email_confirm:true sends nothing];
"Last active" column + email-drift indicator via new GET /api/admin/users
[listUsers()]; audit reuses UPDATE+field_name='email']; no migration;
committed-not-pushed, DO NOT PUSH — Karen next; Lacey then edits the 7
emails self-first + informs users + cleanup commit — 2026-07-05).
...
→ email, else username→email via a case-insensitive `user_profiles`
lookup, legacy `@cqip.local` synthesis kept as a TODO-marked fallback),
`assertTargetIsReadOnlyOrSelf`-guarded `set_email` PATCH action (ordered
two-write, retry-once, loud-fail, no rollback), and the "Last active"
column + email-drift indicator (new `GET /api/admin/users` via
`listUsers()`). No migration — the `email` audit row reuses
`action='UPDATE'` + `field_name='email'` (same CHECK reason as auth.2).
**Rollout still to do (Lacey, post-push):** edit the 7 emails one at a
time (self first), inform users out-of-band. **Cleanup commit** (remove
the legacy `@cqip.local` fallback) comes after all 7 migrate — spec §4.3.
...
**Dual-mode login (`app/login/page.tsx`) [Jenny H3]:** new
`resolveIdentifierToEmail()` — if the input contains `@`, use it as the
email; otherwise resolve username → email via a **case-insensitive**
(`.ilike`) `user_profiles.display_name` lookup, mirroring the reset
flow's intent. Case-insensitive is load-bearing: `display_name` is stored
lowercase by the seed migration (003) but capitalized by the create-user
path, and a case-sensitive `.eq` would send a migrated, capitalized-name
user down the legacy fallback → the exact H3 lockout. The blind
`@cqip.local` synthesis is **kept as the fallback**, marked
...
`@cqip.local` domain, and pre-checks duplicates (case-insensitive; auth
is the real unique authority). **Ordered two-write, no rollback [M3]:**
