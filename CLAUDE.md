...
2-commit chain d046820 → 7c9fec3).
...
- **Phase E — Pulse shell/UX track** (distinct from the C/D feature track):
  - **E1 (Pulse shell: rename + brand pages + nav) — IN FLIGHT** (see §15.5).
    Rename Client Library → Pulse, move the route (with redirect), add
    deep-linkable per-brand pages + a contextual client nav. Render/routing
    only; no migration, no new mutation route, no Jenny.
...
Batch 012 Phase B. DISSOLVED: Batch 010.2
...
### Batch 012 — Phase E1 (Pulse shell: rename + brand pages + nav) — IN FLIGHT

Spec (canonical build doc): `docs/batch-012-phase-e1-pulse-shell-spec.md`.
**Built, committed, NOT pushed** — Karen post-flight next; then Lacey
smoke-tests + deploys. **Render/routing only — NO migration, NO new mutation
route, NO schema change.** No Jenny gate (rename + read-only render + new
read-only page routes; per the "not required for read-only render" rule).

**Locked decisions (do not relitigate mid-build):**
- **Rename** the user-facing area "Client Library" → **Pulse** (nav label, page
  eyebrow; the main view keeps the "Directive Matrix" sub-title). **API routes
  are NOT renamed** (`/api/admin/directives`, `/api/monitoring/*` — concern-
  named, not page-named). **Internal identifiers `lib/client-library/*` keep
  their name** (not user-facing; renaming would churn 6 import sites for no user
  benefit) — a code comment records the intentional divergence.
- **Route move + redirect:** `git mv app/dashboard/client-library →
  ...
  `/dashboard/pulse(/:path*)` (307, temporary) so no bookmark 404s. Nav href
  updated; repo grep for `/dashboard/client-library` in app/components/lib is
  clean.
- **Main Pulse page** = the existing directive matrix, unchanged in content. It
  now **broadcasts its selected project** (sessionStorage key `pulse:project` +
  a `pulse:project` CustomEvent) so the contextual nav mirrors the picker
  WITHOUT `useSearchParams` — the shared nav can't use search params under
  statically-prerendered dashboard pages (`/dashboard/pulse` renders `○`). It
  also seeds its initial project from that stored value (then default, then
  first).
- **Brand page** `app/dashboard/pulse/[projectKey]/[brandCode]/page.tsx`
  (dynamic `ƒ`): URL mirrors `/api/brands/[projectKey]/[brandCode]` so it's
  project-safe (brand_code isn't unique across projects). Read-only: header
  (name/code/project/paused badge), directives section (this brand's status +
  note per active directive, filtered from the same directive dataset the
  matrix loads via the pure `brandDirectiveView` — one source, RLS allows
  authenticated SELECT so no new read endpoint), and a framed **"Convert
  configuration will sync here"** placeholder (E2 fills it). Clean empty states
  ("No directives yet for this brand.", brand-not-found, load error).
- **Contextual client nav** (`components/layout/pulse-client-nav.tsx`, mounted
  in the sidebar): renders ONLY under `/dashboard/pulse`. Lists the current
  project's `is_active` brands — paused greyed-but-linked, inactive excluded,
  alpha by display name (pure `toClientNavItems`). Project source: URL
  `projectKey` on a brand page; else the mirrored picker; else `NBLYCRO`.
  Outside `/dashboard/pulse` the nav is unchanged.
- **Pure logic in `lib/client-library/pulse.ts`** (`toClientNavItems`,
  `brandDirectiveView`, `cellsForBrand`) + `tests/pulse-shell.test.ts` (4 cases:
  brand-cell filter, per-directive view w/ null-when-no-cell, paused-kept/
  inactive-dropped/sorted nav list, empty set). Mirrors the directives/
  monitoring lib+test split.

**Out of scope (placeholders/TODOs only):** Convert config sync (E2), expandable
directive rows w/ comments + lifecycle dates (E3), Jira ticketing (C), public
bug form (D). No schema change.

**Verification:** `tsc --noEmit` clean; `npm run build` green (`/dashboard/pulse`
prerenders `○`, brand page is `ƒ`, old route gone, redirect registered); tests
15/15 (pulse-shell 4 + directives 4 + monitoring 7); ESLint on all new/changed
files → zero findings (the 2 pre-existing `set-state-in-effect` in `nav.tsx` are
the untouched mobile-drawer-close + theme-flip egg cleanup effects, on HEAD
before this batch). No version bump — render/routing only (stays v2.5).

**Karen post-flight: PASS-WITH-FINDINGS** (independently re-verified the
done-definition, no-dead-links repo-wide, rename scope discipline, the
read-only guarantee [both new surfaces only `.select()`], the picker→nav sync
[no hydration risk — first render never depends on the lazy sessionStorage
read], `brandDirectiveView` double-scoping, and the tests; tsc + build + 15/15
+ ESLint reproduced). One LOW folded + two informational observations:
- **LOW-1 — FOLDED (brand→brand stale-content flash).** On brand→brand nav the
  route segment is reused (no remount), so the prior brand's header/directives
  flashed until the new fetch resolved (`loading` was only set false, never back
  to true; data not cleared). Fixed with a render-time staleness gate: a new
  `loadedFor` state (set in every terminal branch of `load()`, all after an
  await → no synchronous setState-in-effect) + `ready = !loading && loadedFor
  === currentKey`; the render shows Loading until the current brand's fetch
  lands. Correctness was already safe (the `cancelled` guard prevented
  contamination); this is the crisp-transition polish.
- **Observation A (as-scoped):** the three API routes still write audit
  ...
  (API routes untouched) + Phase A/B row consistency; revisit only if provenance
  should read "Pulse".
- **Observation B (informational):** deep-linking a brand then "← Pulse" lands
  ...
  brand page never writes its project into the picker). Defensible; a note for
  E2/E3 if brand-page context should become sticky.

**No AC contract surface** (DC-internal) → no CROSS_CLAUDE.md entry.
