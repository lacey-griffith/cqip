...
expecting tighter text would have got WIDER text. Renamed `--tracking-label`.
**Pre-existing and flagged, not fixed:** the same collision already exists for Phase
A's tokens, so `rounded-md` (27 uses), `rounded-lg` (10), `rounded-xl` (25),
`rounded-2xl` (24), `rounded-3xl` (27), `rounded-sm` (5), `shadow-sm` (75) and
`tracking-wide` (14) app-wide resolve to F92 values rather than Tailwind's. Works
today, shipped since 2026-07-17, but **fragile**: moving globals.css into a layer
would flip ~180 call sites at once, silently.
...
the generic no-match state. The brand page's shipped Status group was **reskinned not
...
numerically, but the rendered page cannot be seen from here. The LIGHT theme most
needs a human look, since its values are derived rather than ported — particularly the
five cell shapes at 19px, whose fills are deliberately ~1.15:1 with the border
carrying the signal.
