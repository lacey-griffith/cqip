...
All migrations 001-023 have run against production (022 + 023 applied with
the auth-chain deploy on 2026-07-07).
...
### Batch 005.2 — Coverage Ledger redesign (scoped 2026-07-03; refined 2026-07-05) — IN FLIGHT (committed-not-pushed 2026-07-08; see §15.5)
Jenny pre-flight locked (full visual redesign). Runs after Brand Wellness,
before 010.1 (both touch `coverage/page.tsx`, so 005.2 settles the layout
first). **Known shape:** an **accordion**-style "Coverage Ledger" that
**merges the Batch 010 split Output + Pipeline tables** into one expandable
structure (differences to both). **Spec + design bundle committed**
2026-07-08 (`docs/batch-005.2-coverage-ledger-spec.md` +
`docs/design_handoff_coverage_ledger/` — README, `Coverage Accordion.dc.html`,
screenshots). **DC assessment: no Jenny** (read-only render redesign, no new
mutation surface — spec §9).
**Couples to the Batch 005.1 KPI gauges — render-only** (the 9-card row's
compute layer is untouched; 005.2 only re-renders it as a strip with two SVG
donut gauges).
**Re-homes the Brand Wellness drawer CTA** — deferred to a follow-up per spec
§6 (keep the Reggie drawer + add a "Full detail →" link; do NOT move the BW
CTA or delete the drawer this batch).
**Four scope forks — ALL RESOLVED at build (2026-07-08):**
1. Awaiting-Client-Input tag treatment — **RESOLVED**: one of the four HOLD
   chips (locked set below), matched verbatim via `OVERLAY_TAG_VALUES`.
2. Per-day sparkline — **RESOLVED**: fixed 7-day per-day buckets off the
   existing `milestones` array (new `dailyCounts()` in `queries.ts` +
   `daily7` on `CoverageRow`; new `DeliverySparkline` reusing the sparkline
   module — no new dep).
3. Theming — **RESOLVED**: new `--ledger-*` token layer (dark values
   pixel-match the mock; light values F92-conformant/desaturated for ≥3:1;
   chip fills bright in both themes per the README, near-black text). No inline
   hex, no dark island (§13 r25; `--kpi-longrange-*` precedent).
4. ≤2-copy / sort overlap — **RESOLVED**: header subtitle sources the number
   from `COVERAGE_THRESHOLD` (no hardcoded literal → 010.1 one-touch); the
   five sortable columns (Brand · Delivered 28d · This Wk · Live · Pipeline)
   are separate from the drought copy; default order is status-desc (drought
   floats up) with no "Status" column, so no duplication.

**Pipeline-hold chips — LOCKED v1 chip set (Lacey 2026-07-07; AC-verified
2026-07-07 vs fusion92.atlassian.net, `customfield_12528` "CRO Labels",
multiselect).** The field's full option list: `Needs info` (13949) ·
`On hold` (13948) · `Awaiting client input` (13947) · `Troubleshooting`
(13950) · `Go live` · `Deployment` · `Needs strategy` · `Paused`.
- **HOLD CHIPS (v1):** `Needs info` · `On hold` · `Awaiting client input` ·
  `Troubleshooting`.
- **NOT CHIPS:** `Go live` + `Deployment` (forward states, not holds); · `Needs
  strategy` (unused); · `Paused` (redundant with `is_paused` +
  `Troubleshooting`).
- **Wire `Awaiting client input` VERBATIM** — NOT "Awaiting Client" (that's a
  dead string with no data behind it). Matches the Batch 010 overlay
  convention (`customfield_12528`, exact Jira casing; §13 r33).
Does NOT include the dashboard polish cluster — that's a standalone entry
(different page). Effort MED–LG.
...
**Priority order (updated 2026-07-06 — auth chain DONE; Brand Wellness
slotted after auth-cleanup, ahead of 005.2, per Lacey 2026-07-05; mirrors
CROSS_CLAUDE.md §5):**
Brand Wellness → Batch 005.2 (Coverage Ledger redesign) → Batch 006
(Teams dispatch, EXPANDED) → Batch 010.1 (Pipeline alerts, MERGED —
...
auth-cleanup) plus login-events plumbing have all shipped (§16), clearing
the only operational-risk item on the board (recovery path now exists;
login is email-only). Brand Wellness leads next per Lacey (2026-07-05).
005.2 now precedes 006 (Lacey reorder — was 006-first on 2026-07-03) and
precedes 010.1: 005.2 and 010.1 both touch `coverage/page.tsx`, so the
Ledger redesign settles the layout before 010.1's per-brand pill lands on
it. 010.1 still lands after 006 so pipeline alerts fire Teams pings from
...
Approach-C fix + auth-cleanup], Batch login-events. DISSOLVED: Batch
010.2 and the standalone Path 2 off-by-one item are folded into Batch
010.1; backlog 5.21 is absorbed into Batch 006.)
...
### Batch 005.2 — Coverage Ledger redesign — committed-not-pushed 2026-07-08

Merges the Batch 010 split Output + Pipeline tables on `/dashboard/coverage`
...
**Commits:** (1) `924437a` spec + bundle, docs-only (paths-ignore skips
deploy); (2) the redesign; (3) pre-push fixes from Lacey's smoke + two hygiene
items (this commit — see "Commit 3" below). All committed-not-pushed; Karen
PASS-WITH-FINDINGS on commit 2, re-run pending on commit 3.
...
`daily7: number[]` field on `CoverageRow` (fed by `buildCoverageRows`). All
off the existing full-`milestones` state — no new query/route.

**Components:**
- `components/coverage/sparkline.tsx` — new `DeliverySparkline` export (212×58,
  baseline + polyline + end dot, tokenized via `--ledger-spark-*`; ports the
  bundle's `spark()` math). The old monthly `Sparkline` export (Output-table
  era) was removed in commit 3 once nothing imported it.
- `components/coverage/coverage-gauge.tsx` — NEW. SVG donut with the mock's
  mount animation (arc + numeral count up 0→value, 1000ms ease-out cubic; all
  setState inside rAF so it doesn't trip `set-state-in-effect`;
  prefers-reduced-motion snaps). Renders track-only + '—' when value is null.
- `components/coverage/coverage-ledger.tsx` — NEW. The accordion: 7-col grid
  ...
  stacked bar, status-tag chips or "✓ all clear"). Exports `buildLedgerRow`
  (pure merge of CoverageRow + PipelineBrand → per-stage ready/held; `ready`
  derived from `tickets[]` so a two-tag ticket isn't double-subtracted) so the
  page can sort by the pipeline-derived keys.

**Page (`app/dashboard/coverage/page.tsx`):** the split Output + Pipeline
tables are replaced by `<CoverageLedger>`. New KPI strip (one connected strip,
gap-px hairlines): teal long-range pair · four rolling-window cards · Overall
...
Preserved affordances: `SyncJiraButton` (header, carries its pass/fail pill),
`downloadBrandedXlsx` export, `showPaused` toggle (paused brands get a muted
rail), admin gear → `BrandAdminDrawer`, `AddBrandDrawer`, `ProjectBrandFilter`,
`PipelineStageDrawer` (opened from a per-stage "view →" in the expanded card),
and both footers (orphan-milestone + unresolved-pipeline).

**Reggie drawer (spec §6, Lacey-confirmed path):** kept AS-IS; row click =
expand; a **"Full detail →"** link in the expanded panel opens the Reggie
drawer for that brand. The Brand Wellness CTA stays inside the drawer — NOT
moved, NOT folded in. Drawer deletion / content fold-in / CTA re-home are a
deferred follow-up (mirrors 005.1 Phase 5 discipline).
...
input`). The mock README showed only three (called Troubleshooting "unused") —
§15 supersedes the mock; the amber Troubleshooting chip is wired and renders
when data is present (forward-safe; `OVERLAY_KEYS`/the pipeline route already
carry all four).
...
near-black-text pattern is background-independent + AA either way, so those
tokens are single-value). No inline hex, no dark island.

**Deliberate deviations from Batch 010 (flagged for Karen):**
- The global overlay TOGGLES are DROPPED — the mock has no toggle; the per-stage
  chips in the expanded card replace them (more granular, always visible). Their
  helpers (`OverlayCountBadge`, `UntaggedCountBadge`, `OVERLAY_ACTIVE_CLASS`)
  were removed from `overlay-badge.tsx` in commit 3 (`TagBadge` stays — still
  used by `PipelineStageDrawer`).
- No actual typo was found in the "Show paused brands" label (spec §4.5 asked
  to fix "the known typo"); the copy is already correct, left verbatim.

**Commit 3 — pre-push fixes (Lacey's smoke + hygiene), 2026-07-08:**
- **Header space.** The subtitle rendered "Brands with 2or fewer…" (JSX dropped
  the space around the `{COVERAGE_THRESHOLD}` expression). Switched to a single
  template-literal expression `{`Brands with ${COVERAGE_THRESHOLD} or fewer…`}`
  — keeps the constant interpolation (no literal), space guaranteed, both themes.
- **On Hold → gray** (align to the app's blue=Needs Info / amber=Troubleshooting
  / gray=On Hold convention documented in `globals.css`; loud-solid chip pattern
  kept). Exact hexes: `--ledger-chip-oh` `#FB7185` → `#9CA3AF` (gray-400; single
  value both themes; near-black chip text `#0A0E16` ≈ 6.9:1, AA); `--ledger-seg-oh`
  light `#E11D48` → `#4B5563` (gray-600, ≥3:1 on white), dark `#FB7185` → `#94A3B8`
  (slate-400, bright on the dark bar track).
- **L4 dead-export prune.** Removed monthly `Sparkline` (+ its now-unused
  `useState` import) and the three overlay-toggle-era exports
  (`OverlayCountBadge`, `UntaggedCountBadge`, `OVERLAY_ACTIVE_CLASS` + the now-
  unused `OVERLAY_LABELS` import) — all superseded by the per-stage chips, zero
  importers (tsc-confirmed). `TagBadge` + `OVERLAY_STYLE` stay.
- **L5 ratio dedupe.** Collapsed the duplicated rework-ratio helper into one
  exported `formatReworkRatio()` in `queries.ts`; page + ledger both call it.
  Pure refactor, identical output.

**Verification (commit 3 re-run):** `tsc --noEmit` clean; `npm run build` green
(`/dashboard/coverage` prerenders); `npx tsx --test tests/coverage-kpis.test.ts`
5/5; ESLint on all changed/new files → **zero findings**. (Commit 2's two
build-time findings — gauge `set-state-in-effect`, sort-button `aria-sort` —
were fixed in commit 2 itself.) No version bump for commit 3 (same-batch
follow-up, no new surface — mirrors the Brand Wellness commits-3+4 precedent).

**Open:** §16 shipped entry + §15.5 removal happen ON SHIP (spec §9 / §13 r34),
NOT in these commits. **DO NOT PUSH** — Karen re-run on the commit-3 tree →
Lacey re-smokes (header + On-Hold chip both themes + quick L4/L5 regression) +
pushes.
