...
evidence delivery regressed.** Known, accepted divergence: the number
lives in THREE places and only one moved, so a brand with 3 tests
shows a DROUGHT pill and gets no `alert_events` row — parity is a
one-value config edit, deferred to 010.1, whose §15 entry now carries
the inventory. Karen PASS-WITH-FINDINGS, 5 MEDIUM + 3 LOW all folded,
...
**⚠ THE THREE-COPY DIVERGENCE — DELIBERATE, DEFERRED TO BATCH 010.1, AND USER-VISIBLE
TODAY.** This number lives in three places and **only one moved**:
1. `lib/coverage/queries.ts` — `COVERAGE_TARGET = 4`, `count < target`. **Changed.**
2. **`alert_rules.config.threshold = 2`** — live in prod, `is_active=true` (verified
   2026-08-03). Read by the drought-evaluator cron. **Still 2.**
3. `supabase/functions/drought-evaluator/index.ts` — `DEFAULT_THRESHOLD = 2` fallback
   plus its own `count <= threshold`. **Still 2 and still `<=`.**

**So a brand with 3 tests shows a DROUGHT pill and gets no `alert_events` row.** That is
Lacey's accepted trade — the pill *over*-warns relative to the cron, so nobody loses a
warning they had — not an oversight. **Behavioural parity is a ONE-VALUE config edit**
(`threshold` 2 → 3), because for integer counts `count <= 3` is behaviourally identical
to `count < 4`; an earlier write-up of this batch wrongly claimed the evaluator's `<=`
had to move too (Karen MEDIUM-5). Aligning the evaluator's predicate is still worth
doing — it is what lets a contracted "4 a month" drop in unmodified — but it is an
improvement, not a prerequisite. **The full inventory now lives in the §15 Batch 010.1
entry**, which is the durable home; it previously said "replacing the flat 2/28d
constant" and would have sent a planner hunting for a `2` that no longer exists.
