...
2026-07-31, prod `version: 5870dae`).
...
### Batch 012 — Pulse: restyle core (batch 2 of 4)

**Status:** IN FLIGHT (2026-07-31), committed NOT pushed. Render-only over
already-loaded data + one CSS token pass. **NO migration · NO schema change · NO new
route · NO new mutation surface · NO new fetch · NO new dep → no Jenny (E-track).**
**Version bumped v2.5 → v2.6** — the token block alone wouldn't warrant it, but this
adds two new shared component surfaces (`components/client-library/{status-cell,tab-group}.tsx`),
a new KPI render surface, and a new public lib API (`computeMatrixKpis` + three filter
predicates) while REMOVING an exported one (`countHiddenByStatus`); new component
surfaces plus a changed lib contract is the bar Batch 005.2 bumped for (§13 r23).
Design source: `Pulse_v2__standalone___1_.html` (Claude Design, 2026-07-31) — a
DESIGN source, not a functional spec. **Karen post-flight. DO NOT PUSH.**
Two commits: `dc7fd12` (globals.css tokens, reviewable alone) → `68d8c8a` (reskin).

**Commit 1 — tokens.** Audited first: Phase A had ALREADY shipped the radius /
shadow / tracking / on-orange / orange-hover scale, so those were not duplicated.
Added the 5 Pulse **cell** hues as a NEW `--cell-*` family (not more `--status-*`,
which is the quality-LOG palette per §12 — a log status and a directive-cell status
are different domains that had merely shared hues); dark = the mockup verbatim, light
DERIVED F92-conformant and each ≥3:1 on the card (5.02 / 4.10 / 4.47 / 5.02 / 4.83:1),
reusing existing app values where one fit. Raised the focus ring to ≥3:1 in BOTH
themes **at the token level** via a NEW `--f92-focus-ring` (light #C2410C 5.18:1,
dark #E8833A 5.80:1) rather than changing `--f92-orange`, which is the brand accent;
repointing the unlayered app-wide `:focus-visible` outline moves keyboard focus from
2.76:1 → 5.18:1 across **26 files / 45 call sites' worth of surfaces** without
touching them. Added the missing `--severity-*` DARK overrides, which unblocked
fixing **the two misnamed usages**: `SEVERITY_DOT` was reading `--status-blocked`
(a STATUS token used for SEVERITY) and `--pill-amber-border` (a "-border" token used
as a FILL) while `--severity-*` existed for exactly that.
**A LANDMINE FOUND ONLY IN THE COMPILED CSS:** the one genuinely-new tracking value
was first named `--tracking-tight`, which is ALSO Tailwind v4's own theme token —
with a NEGATIVE value where this one is positive. globals.css is UNLAYERED so it
would have won silently, and anyone later writing `className="tracking-tight"`
expecting tighter text would have got WIDER text. Renamed `--tracking-label`. (Precise claim, per Karen LOW-1: what is true is that
...
Verify by grepping this file for the declaration, not the bundle for the name.)
**Pre-existing and flagged, not fixed:** the same collision already exists for Phase
A's tokens, so the Tailwind UTILITIES `rounded-sm` (5), `rounded-md` (27),
`rounded-lg` (10), `rounded-xl` (25), `rounded-2xl` (24), `rounded-3xl` (27) and
`shadow-sm` (75) app-wide resolve to F92 values rather than Tailwind's — **194 call
sites** in total. Works today, shipped since 2026-07-17, but **fragile**: moving
globals.css into a layer would flip all of them at once, silently.
**Two figures corrected (Karen LOW-2)** — the originals were measured post-batch and
presented as describing a pre-existing state (pre-batch: `rounded-md` 26,
`shadow-sm` 67), and **`tracking-wide` was claimed at 14 when only ONE of those is a
Tailwind utility**. The other 8 are `var(--tracking-wide)` in inline styles, which the
collision cannot affect at all — an inline `var()` reads `:root` regardless of layers.
So that token's real blast radius is 1 site, not 14. Karen's "measured, not asserted"
standard is this batch's own, and these were the only figures that failed it.

**Commit 2 — the reskin.** Cells went dots → **rounded squares** in a new SHARED
`status-cell.tsx` consumed by the matrix, the brand page AND the new legend, so the
three cannot describe a status differently; each status differs in **shape** as well
as hue (solid / half-filled / dashed / hatched / bar) so colour is never the sole
channel. NEW 5-swatch **legend** (there was none) mapping `CELL_STATUSES`, with **no
"has note" swatch** — notes stay non-visual this batch, and a legend entry for an
unrendered indicator would be a lie. **KPI strip: six cards, every value derived** by
`computeMatrixKpis` reusing the existing classifier + `outstandingCount` so it cannot
disagree with the per-row pill; "Largest gap" **omitted rather than rendered inert**
because it is a per-FAMILY number and family grouping does not exist. **Outstanding
semantics unchanged and now test-pinned** (a `todo→in_progress` flip leaves the total
alone — the prod-verified MRH behaviour — while `todo→done` reduces it by one). The
paused-brand exclusion is now said in **exactly one** place (the Brands card) and was
removed from the page subtitle where it also appeared.
**Filters: three independent groups on the matrix**, single-choice within each, AND
across, AND with search. Vocabulary verbatim; "rolled out" appears nowhere.
`State` (derived, all brands) keeps THE VERBATIM GUARD — `state !== 'resolved'`,
never `=== 'active'`. `Status` is ONE CELL's own status and is **not** a rename of
`Resolved`; both exist. Its predicate is has-AT-LEAST-ONE-cell, so a mixed row
matches SEVERAL tabs — intended, test-pinned, and the readout never implies the tabs
partition the rows. `Type` reads the **real `directive_type` column** (the mockup's
title-regex `typeFor()` was NOT ported — it looks like logic, which is what makes it
dangerous), all four always rendered, and an unused tab gets its own copy rather than
the generic no-match state. (Karen LOW-4: an earlier note claimed prod holds only
goal + trigger — false. Verified 2026-08-02: NBLYCRO is goal 75 / trigger 7, but
**SPLCRO already carries 1 `site_area` directive**, so a tab empty on one project is
populated on another. That strengthens rendering all four rather than weakening it.) The brand page's shipped Status group was **reskinned not
rebuilt** (its `open` is still an EXCLUSION of `TERMINAL_CELL_STATUSES`) plus a NEW
Type group — two groups, not three, because one brand means one cell.
...
Type/Status tab did the hiding). ONE honest total, a reset that clears ALL groups and
says so, no per-group attribution (a row can be excluded by two groups at once), ONE
polite live region, ONE reset button. `countHiddenOwedCells` and its amber warning
are PRESERVED — **and were accidentally dropped by the control-bar rewrite, caught
only by an unused-variable lint warning**; losing it would have quietly undone
3363629's MEDIUM-4 fix.
**Two bugs my own checks caught, worth recording:** the lint regression above, and
`hasActiveBrandFilter` first written as "differs from the default", which reported
the WIDEST setting (`All`) as filtered — a test disagreed with me and was right. That
helper was then REMOVED entirely for having no consumer on the brand page (shipping a
tested-but-uncalled export is dead code dressed as coverage); the matrix keeps its
equivalent, where three groups plus a search make it non-redundant.
**Edit target unchanged from 5870dae** on both surfaces: the CELL is the button,
≥24×24 hit area via a zero-layout `after:content-['']`, the brand page's right-hand
label is an INERT `<span>` reskinned as a pill but with NO border / NO hover / NO
ring (border+hover are exactly what made the old chip read clickable), and
hover+focus-visible remains the SOLE affordance per Lacey's recorded decision.
**Out of scope, not partially built:** hover-inspect readout (so cell notes stay
invisible, as today), Change Log widget, per-row resolve dates, family grouping /
flat-list toggle (FLAT ONLY), bulk edit, client page, Convert placeholder; the
monitoring panel is untouched and specifically NOT made collapsible.

**Gates:** tsc 0 · ESLint **zero** on all 9 touched files · **128/128** tests
(112 + 16 new) · build 0 with `/dashboard/pulse` still `○`, the brand page `ƒ`, no new
route entries · **zero inline hex** in all four component files ·
`pulse-client-nav.tsx` diff exactly **0 lines**. Ring rules, the `after:-inset-[3px]`
hit area (`inset:-3px` with `--tw-content:""` present) and every `--cell-*` token
**verified in the COMPILED CSS** — the check that caught two separate bugs this week
and the `--tracking-tight` collision above.
**ONE GATE NOT SELF-SATISFIABLE, stated plainly:** "both `data-theme` values checked
by eye on both surfaces". Token values and every contrast ratio were verified
numerically, but the rendered page cannot be seen from here. Karen narrowed where the
eye-time should go: **`done` next to `blocked` in LIGHT mode** — see MEDIUM-2 below.

**Karen post-flight: PASS-WITH-FINDINGS — 2 HIGH, 2 MEDIUM, 4 LOW, ALL FOLDED.** She
independently reproduced all 24 contrast figures to two decimals, verified all six
compiled-CSS claims (incl. the `@property` registrations, without which the ring would
be dead with every class present), confirmed the scaffolding stayed out (`typeFor`,
`stateFor`, `GROUP_ORDER`, `noteFor`, `DATES` = 0 hits each), found the verbatim guard
**better pinned than claimed** (mutating it fails **9** tests, not 7), re-measured prod
(82 × 16 = 1,312 cells, 0 gaps; 246 paused cells all `n_a`), and confirmed HDCRO's
zero-directive project is handled. **Both HIGHs were cases where a CLAIM outran the
DIFF** — the pattern worth naming:
- **HIGH-1 — dead vocabulary shipped while two documents asserted it hadn't.** "Fully
  rolled out" rendered as a KPI card label and sub-label, **47 lines below this
  batch's own comment saying "'rolled out' is not vocabulary here"**. Worse, it named
  the *same* population the State tab calls `Resolved` — one concept under two names,
  the exact confusion the locked vocabulary exists to prevent, in the most prominent
  readout on the page. Card relabelled **`Resolved`**; the phrase now survives only in
  the comments explaining why it is banned.
- **HIGH-2 — the `--severity-*` dark overrides regressed TEXT contrast on four
  ...
  BACKGROUNDS with `text-white`, and has since long before this batch: brightening
  `critical` took white-on-badge from **4.83:1 (AA PASS) to 2.77:1 (FAIL)** on
  `/dashboard/logs`, both log drawers, and Coverage's "Drought" badge. **Reverted** —
  and the commit-1 rationale for adding them was simply WRONG: the dots never needed
  them, since as non-text indicators they need 3:1 and the LIGHT values already clear
  that on the dark card (critical 3.26 · medium 8.20 · low 6.19). Two consumers
  pulling opposite ways (a dot wants bright, a white-text badge wants dark) and only
  one was measured — against a bar *this batch set for itself* on the focus ring.
  Pre-existing and NOT fixed: badge `high`/`medium`/`low` already fail 4.5:1 in both
  themes, an app-wide badge problem outside a Pulse restyle.
- **MEDIUM-1 — the one surviving paused-brand string was FALSE.** "N paused —
  excluded from Outstanding" is wrong: `computeMatrixKpis` counts every cell and
  `buildMatrixRows` is *required* to see the full paused-included set. It would have
  contradicted the `countHiddenOwedCells` amber warning **on the same screen**, with
  the KPI card being the liar — and it was inherited from the old subtitle and
  *promoted* to a KPI card. Now "columns hidden by default".
- **MEDIUM-2 — "shape as well as hue" was only half true.** The hatch and half-fill
  used the PALE `-fill`, so pattern contrast measured ~1.15:1 in light —
  imperceptible — collapsing the shape channel back onto hue, and `done` vs `blocked`
  are a deuteranope confusion pair (~1.16:1 apparent) while being semantically
  opposite and the two that drive action. Both now derive from the **full-strength
  hue**, taking that channel from ~1.15:1 to **4.10–5.02:1**. (`todo`'s dashed border
  and `n_a`'s bar were already robust — the dash is what rescues to-do vs in-progress,
  whose hues are near-identical under deuteranopia.)
**Karen also recorded, and it is worth keeping:** the brand page's
"no test can stop you" comment on `TERMINAL_CELL_STATUSES` is **accurate as scoped** —
she ran the exact computed-complement mutation and all 128 tests stayed green — while
her *first* attempt (a hardcoded whitelist) IS caught, by this batch's new
"future sixth status defaults to VISIBLE" test. Do not let anyone later "correct" that
comment. **Not a defect of this batch:** the matrix renders `<button disabled>` to
non-admins, so §7's "never a disabled control" holds on the brand page but has never
held on the matrix — byte-identical pre-batch, worth aligning eventually, not a
regression here.
