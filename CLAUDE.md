...
deferred; read-only, no migration, no Jenny; committed-not-pushed,
DO NOT PUSH — Karen next — 2026-07-07).
...
brand's real milestone history. **v2 remains deferred:** rework overlay
(milestones vs sendbacks on one axis), export/share, and multi-brand
compare are NOT in v1 (TODO comments in the component mark them). Note:
Batch 005.2 (Coverage Ledger redesign) will re-home the Brand Wellness
drawer CTA when the drawer is rebuilt.

### Batch 005.2 — Coverage Ledger redesign (scoped 2026-07-03; refined 2026-07-05)
...
structure (differences to both). **Mockup bundle** lives on Lacey's local
machine — **capture the asset paths in this entry when the batch is
written** (placeholder until supplied).
**Couples to the Batch 005.1 KPI gauges — render-only** (the 9-card row's
compute layer is untouched; 005.2 only re-renders it).
**Re-homes the Brand Wellness drawer CTA** (Brand Wellness ships first;
005.2 relocates its entry point into the new ledger).
**Four scope forks to resolve at build time:**
1. Awaiting-Client-Input tag treatment in the merged view (the 4th overlay
   from Batch 010's follow-on — how it renders in an accordion row).
2. Per-day sparkline series (vs the current per-week/monthly counts).
3. Theming approach — bespoke vs strict F92 `--*` tokens (§13 r25).
4. The ≤2-copy / sort overlap question (how the "covered/drought"
   threshold copy and per-column sort coexist without duplication).
...
- **DO NOT PUSH — Karen post-flight next; DC smokes, then pushes.**
