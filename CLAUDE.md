...
count/heatmap that reads it is a later read-only batch; committed-not-
pushed, DO NOT PUSH — Karen next — 2026-07-06),
...
Coverage Ledger / login read-side entries; no migration; committed-not-
pushed, DO NOT PUSH — Karen next — 2026-07-06),
...
committed-not-pushed, DO NOT PUSH — Karen next — 2026-07-07).
All migrations 001-017 have run against production; migrations 022 (Batch
auth.2) + 023 (Batch login-events) await their manual apply before their
code deploys.
...
### Brand Wellness — backlog (slotted first post-auth per Lacey 2026-07-05)
Next DC batch after the auth chain (ahead of 005.2 in the priority order).
**v1** ships per its own commit/spec (scope locked at build time). **v2 is
explicitly deferred:** the rework overlay + export are NOT v1 — they wait
for a later Brand Wellness v2. Note: Batch 005.2 (Coverage Ledger
redesign) re-homes the Brand Wellness drawer CTA, so the two are
sequenced deliberately (Brand Wellness first, then 005.2 relocates its
entry point).
