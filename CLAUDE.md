...
deploy); (2) the redesign (this in-flight commit).
...
- `components/coverage/sparkline.tsx` — new `DeliverySparkline` export (212×58,
  ...
  bundle's `spark()` math). The old monthly `Sparkline` export is now unused on
  the page (the Reggie drawer draws its own recharts bars) — left in place, not
  pruned (out of scope; harmless).
...
- The global overlay TOGGLES + `OverlayCountBadge`/`UntaggedCountBadge` are
  DROPPED — the mock has no toggle; the per-stage chips in the expanded card
  replace them (more granular, always visible). Those two badge exports in
  `overlay-badge.tsx` are now unused (its `TagBadge` is still used by
  `PipelineStageDrawer`); left in place, not pruned.
...
5/5; ESLint on all changed/new files → **zero findings** (the two I introduced
during build — `set-state-in-effect` on the gauge and unsupported `aria-sort`
on the sort button — were fixed before commit).

**Open:** §16 shipped entry + §15.5 removal happen ON SHIP (spec §9 / §13 r34),
NOT in this commit. **DO NOT PUSH** — Karen post-flight → Lacey smoke-tests
BOTH themes (§10) + pushes.
