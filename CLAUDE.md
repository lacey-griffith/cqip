...
- **§2.3 — the readout bar.** Hover AND focus identically, plus the pinned cell.
  ...
  rendering fault.
- **§2.4 — column + row band.** The ROW bands in pure CSS (`group-hover` /
  `group-focus-within` on the `<tr>`), including the sticky first column, which
  carries an opaque `--f92-surface` and would otherwise leave a white notch at the
  start of every highlighted row. The COLUMN needs state, because CSS cannot reach
  the nth cell of every *other* row from a hover on one of them.
- **§2.6 — `<button disabled>` is gone.** `aria-disabled` instead, control stays
  focusable and hoverable, non-admin click pins/unpins. Admins on a **cell-less**
...
derivable today: **82 × 13 = 1,066** under defaults (hide-paused ON → 16 active − 3
...
**A first draft of this claim
said the pure-CSS row band made vertical movement cost zero renders. That was false,
and it was caught pre-review rather than by Karen:** the band needs no state, but the
READOUT does, and it reads the same `hover` whose `directiveId` changes on a vertical
crossing. The CSS band buys simplicity (no `isHotRow` prop threaded through the row),
not renders. It is acceptable because a cheap reconcile a few times a second is
cheap — not because half the crossings were free. If it ever janks, the next step is
a memoized row keyed on `hotBrandId`, which WOULD make vertical crossings nearly free
(no row-level prop changes, so every row bails out and only the readout re-renders) —
which is also why the CSS band remains the right call. NOT done here: extracting the
row would put the sticky editor strip and the E3 seam in the blast radius of a
render-only batch.

**Verification.** tsc 0 · ESLint **zero findings** on all 5 touched/new files ·
**134/134** tests (128 pre-existing + 6 new) · build 0 with `/dashboard/pulse` still
`○`, the brand page `ƒ`, no new route entries. **`hasNote` mutation-verified** —
4 mutations run, all 4 caught (see the §16 entry at ship for the table). NO new
token, so `globals.css` is untouched and §3's "measure every new token on every
consumer" is satisfied vacuously; the marker reuses `--f92-dark` on `--f92-surface`
(17.06:1 light / 12.76:1 dark) with a halo, because a flat mark measured **1.56:1**
on dark `done` and **1.74:1** on dark `blocked`'s stripes. `pulse-client-nav.tsx`
diff **0 lines**. Verified in the **compiled CSS**, not the class list: all three
band rules emit real declarations, the row band beats the sticky column's opaque
background on **both** specificity `(0,2,0) > (0,1,0)` **and** source order, and all
six custom properties in the marker's `box-shadow` shorthand are `@property`-
registered — without which the shorthand is invalid-at-computed-value-time and the
marker would be dead with every class present. The `title` note is proven gone from
the **built chunk**: exactly 4 `title:` remain, all accounted for (2 sort-key
strings, 1 create-POST body field, 1 pre-existing brand `<th>`, 1 pre-existing
severity dot) and **zero** within 120 chars of a note reference.
...
**Status:** built, committed, NOT pushed. **DO NOT PUSH** — Lacey smoke (the two
declared gates + the five eye-gate items above) → Lacey pushes.
