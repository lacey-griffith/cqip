...
- **Phase E — Pulse shell/UX track** (distinct from the C/D feature track):
  ...
  - **E1 follow-on (cross-project client nav) — IN FLIGHT** (see §15.5). The E1
    nav was project-scoped (single-brand clients undiscoverable without
    switching the picker); this makes the client list cross-project — every
    active client, single-brand clients collapsed to one entry. Render/nav only;
    logic in the pure `toClientNavGroups`; reuses the extracted `pulse:project`
    channel; folds Karen E1 observation-B (brand page broadcasts its project).
...
Batch 012 Phase B, Batch 012 Phase E1. DISSOLVED: Batch 010.2
...
### Batch 012 — Pulse E1 follow-on: cross-project client nav — IN FLIGHT

Spec (canonical): `docs/batch-012-pulse-clientnav-spec.md`. **Built, committed,
NOT pushed** — Karen post-flight next; then Lacey smoke-tests + deploys.
**Render/nav only — NO migration, NO new mutation route, NO new page route, NO
schema change; no Jenny gate** (mirrors E1's gate profile). **No version bump.**

**Problem:** E1's client nav is project-scoped (lists only the picked project's
brands, defaulting to NBLYCRO), so single-brand clients (SPL/SPLCRO, and future
Sonrava/ADM) are undiscoverable without switching the project picker. The
per-brand page is already generic — this is purely a nav-discoverability fix.

**Locked decisions (do not relitigate mid-build):**
- **Cross-project client list.** The nav now fetches ALL active projects + ALL
  active brands and renders every client, dropping E1's single-project
  fetch/scoping (and its pickerProject mirror + event listener — no longer
  needed since it shows everything; active highlight is `pathname === href`).
- **Single-brand collapse keyed on `projects.brand_model`** (migration 019):
  `single_brand` → ONE collapsed entry under the client `display_name`, linking
  straight to its brand page; `multi_brand` → a group header (client name →
  matrix, scoped) + its brands. Project with zero active brands is skipped.
- **Pure logic:** new `toClientNavGroups(projects, brands)` in
  `lib/client-library/pulse.ts` returns the ordered render-ready structure
  (groups alpha by project display name; brands alpha; each node carries
  `projectKey` + `brandCode`). The E1 flat helper `toClientNavItems` (+ its two
  now-unused types) was REMOVED — superseded, and keeping it would be a dead
  export; its tested behavior is preserved by the group tests. The renderer is
  intentionally thin (the nav moves to the top in a later batch — only the
  renderer re-skins, not the logic).
- **Reused the existing project handoff — no new mechanism.** The `pulse:project`
  sessionStorage key + CustomEvent + `broadcastPulseProject` were extracted
  verbatim from the E1 page-local copy into a shared client-side module
  `lib/client-library/pulse-project-channel.ts` (NOT pure — touches
  window/sessionStorage — so beside `pulse.ts`, not in it) + a
  `readStoredPulseProject()` helper. The matrix page now imports both (dropping
  its local copies + the `readStoredPulseProject` inline read); the nav's
  multi-brand header `onClick`s `broadcastPulseProject(projectKey)` before
  navigating to `/dashboard/pulse` (sessionStorage carries across → the matrix
  seeds its pick). Removes E1's duplicated "keep in sync" constants.
- **Observation-B ride-along (Karen E1):** the brand page broadcasts the URL's
  `projectKey` on mount (`useEffect(() => broadcastPulseProject(projectKey),
  [projectKey])` — a side effect, not setState) so "← Pulse" opens the matrix on
  the deep-linked brand's client and the cross-project nav highlights
  consistently.

**Discipline kept:** paused = greyed-but-linked, inactive excluded, renders only
under `/dashboard/pulse` (`isUnderPulse`), all setState after the await with a
`cancelled` guard (no set-state-in-effect).

**Tests:** `tests/pulse-shell.test.ts` — the `toClientNavItems` cases were
replaced with `toClientNavGroups` cases (multi-brand grouping + alpha; single
collapse; project alpha; inactive project + empty-brand project excluded; empty
set) alongside the kept `brandDirectiveView`/`cellsForBrand` cases. 17/17 across
the three 012 test files.

**Karen post-flight: PASS-WITH-FINDINGS across two rounds, all FOLDED.**
Independently re-verified (both rounds) tsc/build/17-tests/ESLint, the pure
grouping, the dead-code removal [zero importers of `toClientNavItems`], the
byte-equivalent channel extraction, read-only, and set-state discipline.

- **Round 1 — MEDIUM + LOW, shared root cause: no live consumer for the
  `pulse:project` event.** Removing the nav's listener left the matrix page
  reading the project only once (mount-time `initialLoad`), so a multi-brand
  group-header click *while already on `/dashboard/pulse`* no-op'd (Link to the
  same URL → no remount → no re-read) and the CustomEvent dispatch was dead.
  **Fixed:** the matrix page gained a live `pulse:project` listener that
  re-scopes in place — restoring **page listens, nav + brand-page broadcast** —
  reviving the event consumer. Stale comments corrected.
- **Round 2 — MEDIUM (regression the Round-1 fix introduced): duplicate
  `loadProject`.** `broadcastPulseProject` dispatches synchronously, before
  React commits `setProjectKey`, so the page's OWN broadcasts (initialLoad /
  handleProjectChange) hit the new listener with a stale `projectKey` closure,
  passed the guard, and fired a redundant second load (2× the four Supabase
  queries on every in-page switch + mount — outcome-safe, idempotent, no loop,
  but hot-path waste). **Fixed** (Karen option b): split the channel —
  `writeStoredPulseProject` (persist only, no dispatch) for the page's own
  state-owning producers; `broadcastPulseProject` (persist + dispatch) reserved
  for EXTERNAL producers (nav group headers + brand page) the live listener must
  catch. The page never dispatches to itself → no double load; external events
  still re-scope live. `broadcastPulseProject` now has exactly two callers
  (`pulse-client-nav.tsx`, brand page); the page uses `writeStoredPulseProject`.
- **Minor (not folded, as-scoped):** `toClientNavGroups` single-brand branch
  uses `entries[0]`; a `single_brand` project with >1 active brand (a data
  anomaly vs migration 019's model) drops the extras. Not realistic under the
  schema contract; left defensive-as-is.

**Verification (re-run after the Round-2 fold):** `tsc --noEmit` clean;
`npm run build` green (`/dashboard/pulse` `○`, brand page `ƒ`); tests 17/17;
