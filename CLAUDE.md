...
auth.2). **Rollout still to do (Lacey, post-push):** edit the 7 emails
one at a time (self first), inform users out-of-band. **Cleanup commit**
(remove the legacy `@cqip.local` fallback) comes after all 7 migrate —
spec §4.3.

### Login-activity read side (count + heatmap) — backlog (data captured 2026-07-06)
The `login_events` table + write path SHIPPED as Batch login-events on
2026-07-06 (plumbing only; see §16). This backlog item is the **read
side**: a per-user login count and GitHub-style contribution heatmap on
the admin users page, reading `login_events` (admin-SELECT RLS already in
place). Read-only, no schema change. Open decision (deferred to that
