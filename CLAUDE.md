...
- **MEDIUM-1 — the perf self-correction was INCOMPLETE.** Two stale copies of the
...
**NOT folded — Lacey's call, flagged not buried:** **MEDIUM-4**, three simultaneous
bands in the pinned+focused+hovering state (the row bands from CSS on focus, the
column from the pin, so they point at different cells — §2.4's crosshair asserting
two answers). Cosmetic, but in the state the feature most wants users in. Options:
band the row from state (costs the prop-threading deliberately avoided), suppress
`group-focus-within` while pinned, or blur after pinning. **LOW-5/LOW-7** accepted
as-is (a loose enumeration in a commit message; one marginal double-utterance when
pinning an already-focused cell, where AT behaviour on name-change is inconsistent).
