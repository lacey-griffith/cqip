...
- **Phase E — Pulse shell/UX track** (distinct from the C/D feature track):
  ...
  - **Restyle core (batch 2 of 4) — SHIPPED 2026-08-02** (see §16). Reskinned both
    ...
    folded. v2.5 → v2.6.
  - **Restyle batches 3 and 4 — NOT STARTED.** Batch 3 = the hover-inspect readout
    (which is what finally surfaces cell NOTES; they are deliberately invisible on the
    matrix until then). Batch 4 = the Change Log widget, which needs a NEW `audit_log`
    read — and `audit_log` is **already over the PostgREST 1,000-row cap (1,336 rows as
    of 2026-07-31)**, so it must use `fetchAllPaged()` from the outset. Both were held
    out of the restyle deliberately; do not half-build either.
