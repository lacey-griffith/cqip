...
Lacey in-browser + 3 that have no Worker reader at all; **the 8
plaintext dashboard vars are already GONE and `keep_vars` cannot
restore them — re-adding them is a manual dashboard step**; config +
docs only, no migration, no Jenny; PUSHED + CI-deployed 2026-08-06,
prod `/api/health` reports `version: d21ceea` — commit d21ceea).
...
> does not render `plain_text` regardless. The likeliest account is that the
> 2026-08-06 01:59 local `npm run deploy` (version `aa905ee8`, run to fix the
> token bake) wiped them — the fix destroying the thing it was unrelated to —
...
**Left for Lacey:** re-add the 8 plaintext vars in the dashboard (then
`keep_vars` protects them); confirm which 8 they were, since that list did not
survive anywhere inspectable.
