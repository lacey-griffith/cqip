...
- **Phase E — Pulse shell/UX track** (distinct from the C/D feature track):
  ...
  - **Restyle core (batch 2 of 4) — SHIPPED 2026-08-02** (see §16). Reskinned both
    ...
    folded. v2.5 → v2.6.
  - **Restyle batch 3 (hover-inspect + note surfacing) — BUILT 2026-08-03, IN FLIGHT,
    NOT PUSHED. See §15.5.** Cell notes are no longer invisible on the matrix: a
    rendered marker makes them findable by scanning, and a hover/focus readout bar
    reads one out. Also removed `<button disabled>` for non-admins and dropped the
    native `title` note. Render/interaction only; no Jenny. Karen post-flight owed.
  - **Restyle batch 4 — NOT STARTED.** The Change Log widget, which needs a NEW
    `audit_log` read — and `audit_log` is **already over the PostgREST 1,000-row cap
    (1,336 rows as of 2026-07-31)**, so it must use `fetchAllPaged()` from the outset.
...
(PUSHED + deployed 2026-08-02, prod `version: 2826f4b`).
...
**What shipped (commit 2, code):**
- **§2.1 — `lib/client-library/cell-note.ts`.** `hasNote` + `buildCellReadout`, both
  over ONE private `noteText` normalization, so there is exactly one answer to "is
  this a note". Whitespace-only is NOT a note. The two surfaces previously used a
  bare `cell?.note` truthiness test each and **agreed only by both being wrong in
  the same direction** — an all-whitespace note passed both, which would have drawn
  a matrix indicator promising content the readout had none of, and rendered a bare
  `Note:` label with nothing after it on the brand page. `CellReadout.note` is
  `string | null`, never `''`.
- **§2.2 — the rendered note marker** (`NoteIndicator` in `status-cell.tsx`), the
  load-bearing piece: notes are now findable by SCANNING, where before the only
  discovery path was hovering ~1,300 cells one at a time. Absolutely positioned, so
  zero layout cost — the box does not move, row rhythm is unchanged, and the 24×24
  hit area is untouched; `pointer-events-none` so it can never swallow a click.
- **§2.3 — the readout bar.** Hover AND focus identically, plus the pinned cell.
  Persistent + fixed-height in the DOM for two independent reasons: a live region
  mounted at the moment its content appears is unreliable, and a bar that appeared
  on first hover would shove the grid down exactly when the pointer is over it. An
  absent note says **"No note"** out loud — never a blank region, which reads as a
  rendering fault.
- **§2.4 — column + row band**, including the sticky first column, which carries an
  opaque `--f92-surface` and would otherwise leave a white notch at the start of
  every highlighted row. Shipped with the ROW in pure CSS (`group-hover` /
  `group-focus-within` on the `<tr>`); **commit 4 moved it to STATE** so both axes
  read the same resolved cell — see the MEDIUM-4 fold for why the CSS form could
  disagree with the readout. The COLUMN was always state, because CSS cannot reach
  the nth cell of every *other* row from a hover on one of them.
- **§2.6 — `<button disabled>` is gone**, and `aria-disabled` with it (commit 3);
  the control stays focusable and hoverable, non-admin click pins/unpins, and the
  role difference lives in the accessible name. Admins on a **cell-less**
  cell land on the pin path too — there is no row to PATCH, so pinning is the only
  truthful behaviour there.
- **§2.7 — brand page refactored, NOT rebuilt.** Treatment, placement and the
  `Note: ` prefix are byte-for-byte unchanged; only the source of truth moved.
- **§2.8 — the native `title` is GONE from the cell**, not merely stripped of its
  note: a status-only tooltip would still be a second hover surface for information
  the readout already shows, at a different delay and position.
- Both false comments corrected (`status-cell.tsx` legend, `pulse/page.tsx` cell).

**Two findings from the build, both worth keeping:**
1. **The `sr-only "has note"` hint had never been announced.** It sat inside a
   button that carries `aria-label`, and `aria-label` **overrides** descendant
   content in the accessible-name computation — so the batch-2 comment claiming
   "the screen-reader hint is pre-existing and kept so the information isn't lost
   meanwhile" was **false**: the information *was* lost, for every screen-reader
   user, the whole time. The note now lives in the accessible name itself, which
   also works in browse mode, where a virtual cursor never fires `onFocus` and so
   never triggers a live region at all.
2. **The §2.3-vs-§5 tension is real and was resolved, not papered over.** §2.3 asks
   for a polite live region; §5 requires the readout announce once per cell, not
   twice. On FOCUS the button's own name already speaks directive + brand + status,
   so a live region repeating it is exactly the double announcement §5 forbids.
   Resolution: the region exists and is persistent, and its text is **empty for
   focus-driven changes** — pointer and pin move no focus, so nothing else
   announces for them and the readout is the only voice. Safe under a pin by
   construction: `aria-live` fires on content *change*, and a pin holds the text
   still, so tabbing while pinned re-announces nothing.

**Hover performance (§3 required a stated approach + cost).** `onMouseEnter`, not
`onMouseMove` — state changes only on a cell-boundary CROSSING, so the rate is
bounded by hand speed (a few per second), not by the ~60Hz event loop. That is the
whole of the optimization. **Cost, without the flattering version:** EVERY crossing
reconciles the matrix subtree, vertical as well as horizontal; each cell is a button
plus one styled span.

**The number, DERIVED — and the first draft's "~650 typically" was a STALE figure,
not an estimate.** Rendered cells are exactly `matrixRows.length ×
visibleBrands.length` (the page's two `.map`s). 650 = **50 × 13**, and 50 was the
open-filtered row count recorded when prod held **69** directives; prod is now **82**,
so that product describes a tree that no longer exists — the same
shelf-life-of-a-measurement problem the 2026-07-31 batch hit at 45 minutes. What is
derivable, with the as-of stamped deliberately because losing one is exactly how 650 became false (**82 active directives as of the 2026-07-31 prod probe**; this file records 76 → 82 inside a single batch): **82 × 13 = 1,066** under defaults (hide-paused ON → 16 active − 3
paused), before the status filter removes any row, and **82 × 16 = 1,312** with paused
columns shown. The `open` default only ever *subtracts* rows, so 1,066 is the ceiling
under defaults and the typical figure sits below it by however many directives are
fully resolved — **a count nobody has measured at 82 directives.** Probe it before
sizing the memo follow-on; do NOT scale the old 50/69 ratio, which is how the stale
number got here in the first place. **So judge the follow-on against ~1,066, not 650.**

**A first draft of this claim said the pure-CSS row band made vertical movement cost
zero renders. That was false, and it was caught pre-review rather than by Karen:** the
band needed no state, but the READOUT does, and it reads the same `hover` whose
`directiveId` changes on a vertical crossing — so there was never a free axis. **The
claim is also now MOOT: commit 4 moved the row band to state (MEDIUM-4 option 1), so
no CSS band remains to be called free.** Both axes are state-driven; the follow-on is
a memoized row, recorded against batch 4. Extracting the row was NOT done here —
it would put the sticky editor strip and the E3 seam in the blast radius of a
render-only batch.

**Verification — CURRENT TREE (commit 4). Superseded figures are marked, not left
standing:** tsc 0 · ESLint **zero findings** on all 5 touched/new files ·
**141/141** tests (128 pre-existing + 13 new; 134 at commit 2, 139 at commits 3–4) · build 0 with `/dashboard/pulse` still `○`, the brand page `ƒ`, no new
route entries. **Mutation-verified: 13 run, 13 caught** — 4 on `hasNote`, 4
on the accessible name / announcement bases (all four of which were INVISIBLE until
Karen's MEDIUM-D3 forced a full-string assertion), and the pin-exemption control.
NO new token, so `globals.css` is untouched and §3's "measure every new token on
every consumer" is satisfied vacuously; the marker reuses `--f92-dark` on
`--f92-surface` (17.06:1 light / 12.76:1 dark) with a halo, because a flat mark
measured **1.56:1** on dark `done`'s BORDER and **1.74:1** on dark `blocked`'s
stripes. `pulse-client-nav.tsx` diff **0 lines**.

Verified in the **compiled CSS**, not the class list: `.bg-[color:var(--f92-tint)]`
and `.bg-[color:var(--f92-surface)]` both emit real declarations, and **both
`group-*` band rules are ABSENT from the compiled output** as of commit 4 (the chunk
shrank 74,655 → 74,429 bytes). **SUPERSEDED at commit 4:** the earlier note that
"the row band beats the sticky column on both specificity `(0,2,0) > (0,1,0)` and
source order" described the `group-hover:` variant, which no longer exists — a
state-driven plain class has no such advantage, which is exactly why the sticky
column now uses a mutually-exclusive ternary. All six custom properties in the
marker's `box-shadow` shorthand are `@property`-registered — without which the
shorthand is invalid-at-computed-value-time and the marker would be dead with every
class present. The `title` note is proven gone from the **built chunk**: exactly 4
`title:` remain, all accounted for (a sort-label object, a create-POST body field, a
pre-existing brand `<th>`, a pre-existing severity dot) and **zero** within 120 chars
of a note reference.

**TWO GATES NOT SELF-SATISFIABLE — disclosed, not claimed.** §5's "both themes by
eye" (the light values are derived, so light needs the look most — check the marker
against `blocked`'s hatching and `done`'s fill) and §5's "non-admin path exercised
as a non-admin, not inferred from code" (the pin path, and that the cell is
focusable and hoverable without being actionable). Both are Lacey's.

**One observation for Karen, not a defect:** a read-only user now has one tab stop
per cell — ~1,300 on NBLYCRO — where `disabled` previously gave them zero. That is
exactly what §2.6 locks ("exactly one tab stop per cell either way") and it is what
makes focus-driven inspection work, and admins have always had those stops. But a
grid with 1,300 tab stops is a known anti-pattern whose conventional fix is a roving
tabindex over `role="grid"`. Out of scope here; worth a decision before batch 4.

**KAREN POST-FLIGHT 2026-08-03 — PASS-WITH-FINDINGS: 1 HIGH · 5 MEDIUM · 7 LOW.**
Recorded here deliberately, because the `c58364c` review was unsubstantiable three
weeks later (no commit referenced the sha, §15.5 held no verdict, §16 had no entry)
and had to be re-run from scratch. Fold state: commit 3 of the chain. She
independently reproduced all four contrast figures to three decimals, all four
`hasNote` mutation results, the specificity math, the six `@property`
registrations and the `title` proof — all correct — and confirmed the batch's
headline a11y finding is **stronger** than stated: the dead `sr-only` span was not
merely unannounced but unreachable by *every* mechanism, since AT treat a button as
atomic for the virtual cursor.

**FOLDED in commit 3:**
- **HIGH-1 — a pin filtered off screen killed the readout entirely.** `pinned ??
  hover` won unconditionally, so once a pin stopped resolving against
  `matrixRows`/`visibleBrands` the memo returned null and **never consulted
  `hover`** — the bar went dead on every visible cell, recoverable only by
  noticing an orphaned "Unpin" link beside an empty state. Reachable by the
  feature's own primary flow: pin a cell to read its note (the only touch path),
  then search for the next one. Now the two candidates are *resolved in order*, so
  the "readout describes a visible cell" invariant holds AND an unresolvable pin
  degrades to hover instead of disabling the deliverable. Karen's diagnosis of the
  comment was right and is the batch's own lesson turned on itself: the mechanism
  was described correctly and its consequence omitted.
- **MEDIUM-1 — CLOSED in commit 4. The perf self-correction was INCOMPLETE.** Two stale copies of the
  false claim survived, both added by this batch's own commit, and both reproduced
  the exact flattering framing the correction repudiates. Also a spec violation:
  §1 says *"do not leave a comment standing that the batch falsifies"* — applied to
  batch 2's comments and then reintroduced against my own.
- **MEDIUM-2 — `aria-disabled` dropped (DC's call; the spec is wrong here).** ARIA
  defines it as "not editable or otherwise operable", and this control IS operable:
  the click pins. It announced "unavailable" on a working button and contradicted
  the cell's own name, which ends "(activate to pin)" — and NVDA/JAWS can skip
  unavailable controls, degrading exactly the population §2.6 exists to serve. The
  role difference now lives in the accessible name. `cursor-default` went with it:
  same lie, mouse channel.
- **MEDIUM-3 — the touch path was undiscoverable on touch.** The only instruction
  read "Hover or focus a cell", naming two interactions the device lacks, on the
  surface whose whole justification is that tapping is the note path when there is
  no hover. Now interaction-neutral.
- **MEDIUM-5 — the accessible name is now a TESTED pure function**
  (`buildCellAriaLabel`), not 8 lines of concatenation mid-JSX, and it consumes
  `readout.note` rather than doing its own second `.trim()`. It is the only
  announced note path on focus and in browse mode, and inline-in-JSX is precisely
  how the last one rotted unnoticed.
- **LOW-1/2/3/4/6** — the legend swatch geometry claim (it is NOT the same
  position: 24px ancestor in the grid vs 13px in the legend), the marker's overlap
  of the box corner, the `1.56:1` figure being `done`'s **border** rather than its
  fill (whose composite passes at 8.79:1), specificity-not-source-order in the code
  comment, and `aria-hidden` on the Unpin button, which sat inside the `aria-atomic`
  region and appended "Unpin button" to every pin announcement.

**MEDIUM-4 — FOLDED in commit 4, option 1 (Lacey's call 2026-08-03): the row band
moved from pure CSS to STATE, so both crosshair axes derive from the same resolved
cell the readout describes.** Lacey's push-back on my own three-option summary was
correct and is worth keeping, because it rejects the two cheap fixes on a structural
argument rather than a taste one: **(2) suppress `group-focus-within` while pinned
and (3) blur after pinning do not collapse the disagreement, they relocate it.** Pin
A then hover B — `inspected` prefers the PIN, so the column is A's; suppressing the
focus variant only stops row A self-banding, and `group-hover` still bands **row B**.
Both options leave row B + column A, crosshairing a cell that is neither pinned nor
hovered nor the one the readout is describing. The defect was structural: one axis
pointer-driven CSS, the other state-driven with the pin winning, so they *cannot*
agree. Only option 1 has them read one source.
- `isHotRow` is threaded from `inspected`; `group`, `group-hover` and
  `group-focus-within` are gone — **verified absent from the COMPILED CSS**, not just
  unused in source.
- **Dropping `group-focus-within` costs nothing**, because `onFocus` already feeds
  `hover`: a tabbed-to cell resolves through `inspected` and bands its row from
  state. The keyboard band survives the change.
- **The sticky column's two backgrounds are now MUTUALLY EXCLUSIVE via a ternary,
  and that is load-bearing.** Both are plain `bg-*` utilities at (0,1,0), so if both
  were ever present the winner would be decided by Tailwind's **emission order** —
  and measured, `tint` is emitted *after* `surface` (byte 24442 vs 24370), so the
  column would band on EVERY row. The old `group-hover:` variant could be layered
  safely only because `:is(:where(.group):hover *)` made it (0,2,0); a state-driven
  plain class has no such advantage. The ternary makes the hazard unconstructable.
- **Render cost accepted, per Lacey:** both axes are state-driven now, so vertical
  crossings cost a render too. It is `mouseenter`-driven, not 60Hz. The row component
  was deliberately NOT extracted — keeping the sticky editor strip and the E3 seam
  out of a render-only batch's blast radius stands. **The memoized-row follow-on is
  recorded against batch 4 and must be judged against ~1,066 (82 × 13 under
  defaults), NOT 650.**
- **Keyboard: two questions, two treatments, deliberately not merged.** The
  `focus-visible` ring stays the "where am I" cue (per the F1 ring-as-sole-affordance
  decision); the band answers "what is the readout describing".

**LOW-5/LOW-7** accepted as-is (a loose enumeration in a commit message; one marginal
double-utterance when pinning an already-focused cell, where AT behaviour on
name-change is inconsistent).

**On the tab stops (my question #13) Karen said ship it, and corrected my framing.**
"Admins always had those stops" elides that each admin stop *does something*, while
a read-only keyboard user could previously Tab past the whole table in one step and
now cannot — a real traversal regression not shared with admins. Her strongest
argument for shipping is one I had not made: the note now lives in the **accessible
name**, so screen-reader users reach it in **browse mode, which does not use Tab** —
making this a sighted-keyboard-user problem, not an AT one. Interim mitigation she
proposes is a **skip-the-matrix link** (a handful of lines, no roving-tabindex blast
radius); the `role="grid"` roving tabindex stays the batch-4 decision. Recorded in
§15 against batch 4.

**She also confirmed nothing was hidden behind the two disclosed gates**, and
narrowed where the eye-time goes, in her priority order: (1) the **legend swatch**,
both themes — likeliest to look wrong; (2) **light theme, `blocked` and `todo`
note-bearing cells** — does the white halo punch a hole in the hatch/dash, light
being where the halo is least necessary and most intrusive; (3) the **pinned +
focused + hovering** state (MEDIUM-4); (4) **banded rows in light**, where the halo
is 1.07:1 against `--f92-tint` and effectively vanishes (harmless — the dot is
15.95:1 on tint — but "the ring is the card" is only true off the band); (5)
**non-admin on touch**, confirming a tap pins.

**KAREN DELTA RE-LOOK 2026-08-03 (`986c20e..cfd1f3d`) — CONFIRMED, 3 MEDIUM · 4 LOW,
all folded in commit 5.** She confirmed the MEDIUM-4 fold is clean and correct, that
both axes provably read one source (including when `inspected` is null, mid-edit, and
during the HIGH-1 fallback), that HIGH-1 is closed and leaving a dangling pin set is
the right call, and that the `group-*` rules are genuinely absent from the compiled
output rather than merely unused. **She also corrected my own option summary in the
opposite direction from Lacey's:** option 2 would not merely have relocated the
disagreement, it would have made the pinned case *worse* — as shipped, pin A + focus A
+ hover B gave a CORRECT crosshair at (A, colA) plus one spurious row; suppressing
`group-focus-within` deletes row A, i.e. removes the correct intersection and keeps
only the wrong one. Option 1 was the only fix.

- **MEDIUM-D1 — the "Pinned" badge lied in exactly the state the HIGH-1 fix created.**
  It tested `pinned` ("a pin exists") rather than `inspected?.isPinnedCell` ("the cell
  you are reading IS the pin"), so after the fallback the bar correctly described the
  HOVER cell with a "Pinned" badge beside it — **and the announcement made no such
  claim, because it already asked `isPinnedCell`, so the visible badge contradicted
  the spoken text with the visible one lying.** Same shape as batch 2's HIGH-2: two
  consumers of one fact, only one wired. `isPinnedCell` had exactly one consumer when
  it was created for exactly this distinction. Fixed, plus an honest "pinned cell is
  hidden by the current filters" hint so the Unpin control isn't left next to an
  unrelated readout.
- **MEDIUM-D2 — §15.5 and the footer contradicted themselves, stale half first.** Four
  commit-2 claims survived commit 4 unrevised: "all three band rules emit", the
  `(0,2,0) > (0,1,0)` specificity argument, "no `isHotRow` prop threaded … the CSS
  band remains the right call", and `134/134`. **This is MEDIUM-1's own rule applied
  to code comments and not to the prose above them** — and it matters more here,
  because §0 tells the next session to trust this file as ground truth and the footer
  is read first. Superseded figures are now marked as superseded rather than deleted.
- **MEDIUM-D3 — the accessible-name contract was PARTIAL: the brand was unpinned.**
  Every assertion was a fragment matcher, so dropping `brandLabel` from either
  function's base, reordering the name, or swapping the em dash for a hyphen all
  produced **ZERO** failures. On a 13-column grid "Clicks Print Offer: To do" is
  unusable — the column header is not part of the button's accessible name, so a
  browse-mode user would not know which brand they were on. Two `assert.equal`s on
  full expected strings now pin base, separators and ordering at once; all four
  previously-invisible mutations are caught, verified by re-running them.
- **LOW-D1 — a dead assertion, and the same family as the weak test mutation caught.**
  `!said.endsWith('. ')` could not fail while the `endsWith('No note')` above it
  passed (one suffix per length). Found by deleting it and seeing nothing change.
  Removed, with the reason recorded: an assertion that cannot fail while its
  predecessor passes is not a check.
- **LOW-D2 — "82" shipped without its vintage**, which is the exact sin item 3 was
  correcting one level up: 650 became false because `50 × 13` lost its as-of. Now
  stamped 2026-07-31 in both §15.5 and the footer.
- **LOW-D3** closed by MEDIUM-D3's full-string equality. **LOW-D4** accepted as-is
  (the marginal double-utterance when pinning an already-focused cell survives the
  extraction; the comment does not claim otherwise).

**Unremarked by me and worth keeping (Karen):** the mutually-exclusive ternary
preserves the sticky column's OPACITY in both states only because `--f92-tint` is
opaque hex in both themes (`#FEF6EE` / `#2A2F45`). Had either token been an `rgba()`,
rows would show through the sticky column while scrolled in the hot state. That is
load-bearing and was accidental.

**Status:** built, committed, NOT pushed. **DO NOT PUSH** — Lacey smoke → Lacey
pushes. **Eye-gate, re-narrowed after the delta:** (1) the **legend swatch**, both
themes — likeliest to look wrong; (2) **light theme, `blocked` and `todo` note-bearing
cells** — does the white halo punch a hole in the hatch/dash; (3) **what pinning feels
like when the pointer is elsewhere** — the crosshair stays on the pin and only the
hovered button rings, so confirm it reads as "pinned there, pointing here" rather than
a stuck page (this REPLACES the old two-bands item, which option 1 made structurally
impossible); (4) **banded rows in light**, where the halo is 1.07:1 on `--f92-tint`
and effectively vanishes — harmless, the dot is 15.95:1 on tint, but "the ring is the
card" is only true off the band; (5) **a hot row with its editor open**, which now
shares `--f92-tint` with the row and reads as one block; (6) **non-admin on touch**,
confirming a tap pins.
